...
class JsonSequenceRepository implements SequenceRepository
{
    ...
    public function generate(
        ...
        [$start, $end] = FiscalYear::spanForDate($date, is_array($settings['general'] ?? null) ? $settings['general'] : []);
        ...
        $lastFromSettings = Str::of(Data::string($rawSaved))
            ->whenStartsWith($match, fn ($s) => $s->after($match))
            ->__toString();
        $lastFromSettings = is_numeric($lastFromSettings) ? (int) $lastFromSettings : 0;
    ...
    public function update(
        ...
        $numericPart = Str::of($newNumber)->match('/(\d+)$/')->__toString();
        ...
        $storedNumeric = Str::of(Data::string($rawStored))->match('/(\d+)$/')->__toString();
