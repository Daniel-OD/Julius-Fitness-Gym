<x-layouts.app :title="__('app.actions.new', ['resource' => __('app.resources.members.singular')]) . ' · ' . config('app.name')">
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <nav class="mb-2 flex items-center gap-2 text-sm text-gray-500">
                    ...
                    <span>/</span>
                    <span class="text-gray-900">{{ __('app.actions.new', ['resource' => __('app.resources.members.singular')]) }}</span>
                </nav>
    ...
    <form action="{{ route('web.members.store') }}" method="POST" enctype="multipart/form-data"
        ...
        <x-ui.card title="Poză profil" subtitle="Opțional — JPG sau PNG">
            <label
                class="flex cursor-pointer flex-col items-center justify-center rounded-xl border border-dashed border-gray-200 bg-gray-50 px-6 py-8 transition-colors hover:border-brand-300 hover:bg-brand-50/50">
                <svg class="mb-2 h-8 w-8 text-brand-600" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                    stroke-width="2">
                    <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" />
                    <polyline points="17 8 12 3 7 8" />
                    <line x1="12" x2="12" y1="3" y2="15" />
                </svg>
                <span class="text-sm font-medium text-gray-700">Alege imagine</span>
        ...
        <x-ui.card title="{{ __('app.fields.details') ?? 'Date personale' }}">
            <div class="grid gap-5 sm:grid-cols-2">
                ...
                <div>
                    <label for="goal" class="mb-1.5 block text-sm font-medium text-gray-700">{{ __('app.fields.goal') ?? 'Obiectiv' }}</label>
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea id="address" name="address" rows="2"
                        class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500">{{ old('address') }}</textarea>
                </div>
            </div>
        </x-ui.card>
